...
final class Level {
	...
	public static function add_taxonomy( array $taxonomies ) : array {
		...
		$taxonomies['level'] = array(
			'key'    => 'matador-levels',
			'single' => _x( 'title', 'Level Singular Name.', 'matador-extension-custom-pg' ),
			'plural' => _x( 'titles', 'Levels Plural Name.', 'matador-extension-custom-pg' ),
	...
	public static function save_levels_taxonomy_terms( stdClass $job, int $wpid, Bullhorn_Import $bullhorn ) : void {
		...
		if ( ! empty( $job->customText6 ) ) {

			$value = $job->customText6;
